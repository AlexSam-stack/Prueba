...
$("#tabla_area").on("click", ".editar", function () {
  ...
  } // Permite llevar los datos cuando es tamaño celular y usas el responsive de tbl_area
  ...
  $.ajax({
    ...
    if (resp > 0) {
      if (resp == 1) {
        ...
        Swal.fire(
          "Mensaje de Advertencia",
          "El área ingresada ya se encuentra en la base de datos",
          "warning"
        );
      }
    } else {
...
function Modificar_Area() {
  ...
  $.ajax({
    ...
    if (resp > 0) {
      ...
      return Swal.fire(
        "Mensaje de Error",
        "No se completó la modificación",
        "error"
      );
    }
  });
